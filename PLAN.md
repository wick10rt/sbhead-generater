...
- 2026-05-26｜SR 軟化（raw × α 混合）｜使用者反映 sr 4096 輸出「黑色輪廓線粗黑 + 整體飽和度/對比偏高」，raw 不硬、不同尺寸輸入都一樣硬 → 判定為 x4plus_anime_6B 模型 baseline GAN 偏好，與 enhance 參數、SR pass 次數、fp16/fp32 都無關。修正：`utils/avatar_output.py` 新增 `SR_BLEND_ALPHA = 0.35`，`save_paired_avatar` 在 sr 寫檔前用 `cv2.addWeighted` 混入 35% raw、保留 65% sr，純 CPU 運算（不碰 GPU、不增 VRAM、零 OOM 風險）。裁切 ≥ 4096 時 sr 與 raw 是同一物件直接跳過混合。先採方案 1（直接覆蓋 sr/、不新增 blend/ 資料夾）試水溫；α=0.35 為起始值，視覺效果不滿意再調。
