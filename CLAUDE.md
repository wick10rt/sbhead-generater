...
驗證輸入（路徑存在、格式為 JPG/JPEG/PNG）
       ↓
dghs-imgutils 偵測動漫臉部
  → 無臉：sys.exit(1) + 錯誤訊息
  → 有臉：取面積最大的 bbox
       ↓
依 bbox 裁切頭像（擴大含頭髮與肩膀，裁成正方形）
       ↓
enhance（銳化 + 降噪 + 對比，永遠執行）
       ↓
判斷裁切後尺寸
  → < 1024：Real-ESRGAN 超解析度放大（失敗則 fallback cv2.resize）
  → ≥ 1024：直接 resize
       ↓
Resize 至 1024×1024
       ↓
存到 outputs/output.png（有衝突則自動加編號）
       ↓
終端機印出完成訊息與輸出路徑
```
...
- 函式：`save_avatar(image: np.ndarray, outputs_dir: Path) -> Path`
- 輸出固定 1024×1024 PNG
- 命名：`output.png` → `output(1).png` → `output(2).png`…
- 自動建立 `outputs/` 資料夾
