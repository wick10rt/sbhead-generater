...
使用者在本機端透過終端機執行 Python 程式，輸入一張動漫角色圖片路徑後，系統自動偵測臉部、裁切頭像、進行畫質增強與 AI 超解析度放大，輸出 4096×4096 PNG 大頭貼。
...
| SR 模型 | `RealESRGAN_x4plus_anime_6B.pth`，放 `weights/`；**tile=0 不分塊**（輸入邊長 > 2048 自動降為 tile=2048 防 OOM）；**fp32**（追求最高品質） |
| 路徑處理 | pathlib + PIL |
| 中間結果 | 不儲存 |
...
- 優先使用 CUDA GPU
- **品質設定**（3090 24GB 升級後）：`tile=0` 不分塊推論，邊緣零拼接縫；但為避免 3090 在第二次 SR pass（輸入可達 3200+）OOM，加入安全機制：**每次呼叫 SR 前檢查輸入邊長，若 > 2048 則改用 tile=2048**（仍幾乎無縫）。`half=False` 永遠用 fp32 推論，動漫平塗區更乾淨
