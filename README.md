...
整個系統以本機端的命令列工具方式呈現，使用者只需在終端機輸入一行指令並指定圖片路徑，程式便會自動偵測畫面中所有角色的臉部位置、裁切頭像區域、進行畫質增強與 AI 超解析度放大，並針對每一張臉輸出「未做超解析度」與「做完超解析度」兩個 4096×4096 版本（方便比對 AI 放大後的畫質差異），外加一個由 sr 版縮小而成的 2048×2048 最終成品。整個過程不需要開啟任何網頁或視窗介面，操作單純直覺。
...
裁切完成後，程式自動進行適度的銳化與降噪，避免動漫線條被過度處理（原本的 CLAHE 局部對比度均衡會把臉頰等柔和漸層區壓暗成塊狀髒污，已移除）。接著針對每張臉產出三個檔案：「raw 版」直接以傳統 resize 縮放至 4096×4096；「sr 版」則根據裁切後尺寸做判斷，若小於 4096 便使用 Real-ESRGAN（採用適合動漫圖片的 RealESRGAN_x4plus_anime_6B 權重）進行 AI 超解析度放大，由於 x4plus 模型一次只能放大 4 倍，若一次仍不足會連續執行多次直到尺寸 ≥ 4096，再 resize 至 4096；若已大於等於 4096 則同樣直接 resize。raw 與 sr 兩版都是 4096×4096，分別存放於 `outputs/raw/` 與 `outputs/sr/`，方便對照 AI 上採前後的畫質差異——尤其是裁切尺寸較小的情況，raw 版被傳統 resize 強行拉大會明顯模糊，恰好凸顯 SR 版的優勢。最後再由 sr 版的 4096 以 INTER_AREA 縮小成 2048×2048 的最終成品，存放於 `outputs/2048out/`（縮小本身即天然抗鋸齒、邊緣更平滑），三邊檔名同步編號。
...
4. 對每一張偵測到的臉依序執行下列步驟：
   ...
   2. 自動進行銳化、降噪
   3. **raw 版**：直接 resize 至 4096×4096
   4. **sr 版**：若裁切後 < 4096px 則連續跑 Real-ESRGAN 超解析度放大直到 ≥ 4096，再 resize 至 4096×4096；若 ≥ 4096px 則同 raw 版直接 resize
   5. **2048out 成品**：由 sr 版 4096 以 INTER_AREA 縮小至 2048×2048
   6. 將 raw / sr / 2048out 三個版本分別輸出至 `outputs/raw/`、`outputs/sr/`、`outputs/2048out/`（同名同編號）
...
最終以錄影方式呈現，準備數張不同的 Bang Dream 角色圖片（包含單人與多人合照），展示自動臉部偵測、裁切以及超解析度放大的效果。程式執行時會在終端機逐步印出每張臉的處理進度，並將 raw / sr / 2048out 三個版本分別存到 `outputs/raw/`、`outputs/sr/`、`outputs/2048out/`。錄影時會把同編號的 raw 與 sr 並排比對，凸顯傳統 resize 與 AI 超解析度放大之間的細節差異，並展示多臉合照能一次處理多張角色的效果。
...
輸出結果會分別存到 `outputs/raw/`、`outputs/sr/`、`outputs/2048out/` 三個子資料夾：

- `outputs/raw/output.png`：未做超解析度的版本（僅裁切 + enhance + 傳統 resize），4096×4096
- `outputs/sr/output.png`：做完 Real-ESRGAN 超解析度的版本，4096×4096
- `outputs/2048out/output.png`：最終成品，由 sr 版 4096 以 INTER_AREA 縮小，2048×2048
...
>
> **注意 2**：4096×4096 PNG 單檔約 5–20 MB、2048×2048 約 2–6 MB；每張臉產出三檔，多臉合照單次執行可能輸出上百 MB。請留意磁碟空間。
>
...
└── outputs/                         ← 執行結果（不進 git）
    ├── raw/                         # 未做 SR 的版本（4096）
    ├── sr/                          # 做完 SR 的版本（4096，與 raw/ 同名同步）
    └── 2048out/                     # 最終成品（2048，由 sr 縮小，同名同步）
```
